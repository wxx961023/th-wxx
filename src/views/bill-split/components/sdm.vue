              ...
              if (personColumnIndex !== -1) {
                ...
                if (validDataFromRow4.length > 0) {
                  ...
                  validDataFromRow4.forEach(item => {
                    ...
                    // 使用人名作为分组名称，每个人一个单独的账单
                    const groupName = personName;
...
const generateFileName = (
  ...
  // 森达美直接使用人名 + 日期
  const finalFileName = `${groupName}${dateRange}`;
...
const generateGroupedExcelFiles = async () => {
  ...
  try {
    ...
    for (const companyGroup of groupInfo) {
      ...
      // 森达美直接使用人名作为公司名称
      console.log(`分组信息:`, companyGroup.groupName);
      ...
      templateWorksheet.eachRow((row, rowNumber) => {
        row.eachCell((cell, colNumber) => {
          ...
          if (typeof cellValue === "string") {
            ...
            // 替换联系人信息（森达美直接使用人名）
            if (cellValue.includes("收件人：")) {
              cellValue = cellValue.replace(
                /收件人：[^，,\n]+/,
                `收件人：${companyGroup.groupName}`
              );
              hasChanges = true;
            }
            // 森达美不替换电话号码，保持原有电话号码
        ...
        if (personColumnIndex !== -1) {
          ...
          const personHotelData = hotelData
            ...
            .filter((row: any[], index: number) => {
              ...
              // 森达美分组逻辑：根据人名匹配
              const groupName = personText.trim();
      ...
      console.log("在生成Excel前最终设置sdm文件的背景色和边框...");
...
<template>
  <div class="bill-split-container">
    <div class="bill-split-content">
      ...
      <!-- 数据展示区域 -->
      <div v-if="showData && getGroupCount() > 0" class="data-section">
