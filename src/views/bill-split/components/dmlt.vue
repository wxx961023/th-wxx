        ...
        .then(() => {
          ...
          sheetProcessors.forEach(processor => {
            ...
            if (jsonData.length > 2) {
              ...
              // 在第三行中查找包含人名关键字的单元格
              const personColumnIndex = thirdRow.findIndex(
                (cell: any) =>
                  cell && cell.toString().includes(processor.personKeyword)
              );

              if (personColumnIndex !== -1) {
                const personColumnName =
                  headers[personColumnIndex] || `第${personColumnIndex + 1}列`;
                const personData = jsonData.map((row, index) => ({
                  行号: index + 1,
                  [processor.personKeyword]: row[personColumnIndex]
                }));

                console.log(
                  `\n========== "${processor.personKeyword}"列信息 ==========`
                );
                console.log(
                  `找到"${processor.personKeyword}"单元格位置: 第3行，第${personColumnIndex + 1}列`
                );
                console.log(`对应表头列名: ${personColumnName}`);
                console.log(`该列完整数据:`, personData);

                // 过滤掉空值，只显示有数据的行
                const validPersonData = personData.filter(
                  item =>
                    item[processor.personKeyword] &&
                    item[processor.personKeyword].toString().trim() !== ""
                );
                console.log(
                  `\n有效数据（共${validPersonData.length}条）:`,
                  validPersonData
                );

                // 对行数大于等于4的有效数据进行分组处理
                const validDataFromRow4 = validPersonData.filter(
                  item => item.行号 >= 4
                );
                ...
                if (validDataFromRow4.length > 0) {
                  ...
                  validDataFromRow4.forEach(item => {
                    const personName = item[processor.personKeyword]
                      .toString()
                      .trim();
                  ...
                  groups.forEach((groupItems, groupName) => {
                    ...
                    groupItems.forEach(item => {
                      console.log(
                        `  行${item.行号}: ${item[processor.personKeyword]}`
                      );
                    });
                  });
                ...
                console.warn(
                  `在第三行中未找到'${processor.personKeyword}'单元格`
                );
              }
            }
...
const getGroupInfo = () => {
  ...
  if (allSheetData.value.hotel && allSheetData.value.hotel.length > 0) {
    ...
    hotelGroups.forEach(group => {
      if (!personGroups.has(group.groupName)) {
        const generatedFileName = generateFileName(group.groupName);
        personGroups.set(group.groupName, {
      ...
      const personGroup = personGroups.get(group.groupName)!;
      personGroup.hotelInfo = {
        count: group.count,
        rowRange: group.rowRange
      };
      personGroup.totalCount += group.count;
    });
  }
  ...
  if (
    ...
    internationalHotelGroups.forEach(group => {
      ...
      const personGroup = personGroups.get(group.groupName)!;
      personGroup.internationalHotelInfo = {
        count: group.count,
        rowRange: group.rowRange
      };
      personGroup.totalCount += group.count;
  ...
  if (
    ...
    internationalFlightGroups.forEach(group => {
      ...
      const personGroup = personGroups.get(group.groupName)!;
      personGroup.internationalFlightInfo = {
        count: group.count,
        rowRange: group.rowRange
      };
      personGroup.totalCount += group.count;
  ...
  const result = Array.from(personGroups.values());
  return result;
};
...
const processSheetData = (
  ...
  // 重新计算分组数据
  const personColumnIndex = (sheetData[2] as any[]).findIndex(
    (cell: any) => cell && cell.toString().includes(personKeyword)
  );

  if (personColumnIndex === -1) {
    console.warn(`在${sheetName}中未找到${personKeyword}列`);
    return [];
  }
  ...
  const validDataFromRow4 = sheetData
    ...
    .filter((row: any[], index: number) => {
      const personValue = row[personColumnIndex];
      if (!personValue || personValue.toString().trim() === "") {
        return false;
      }

      const personText = personValue.toString();
      ...
      const isSummaryRow = summaryKeywords.some(keyword =>
        personText.toLowerCase().includes(keyword.toLowerCase())
      );

      // 过滤纯数字（可能是金额合计）
      const isPureNumber = /^\d+(\.\d+)?$/.test(personText.trim());

      // 过滤空值、特殊字符
      const isEmptyOrSpecial =
        personText.trim() === "" ||
        /^[\-_=+]+$/.test(personText.trim()) ||
        personText.length < 2;

      // 如果是空值、合计行、纯数字或特殊字符，不进行分组处理
      if (isSummaryRow || isPureNumber || isEmptyOrSpecial) {
        console.log(
          `跳过非数据行: 行号${index + 4}, 内容: ${personText}, 类型: ${
    ...
    .map((row: any[], index: number) => ({
      行号: index + 4,
      [personKeyword]: row[personColumnIndex],
      完整行数据: row
    }));
...
// 导入配置
import companyConfig, { personConfig } from "../companyConfig";
...
const generateFileName = (
  ...
  const personInfo = personConfig.getPersonInfo(groupName);

  const dateRange = generateCurrentMonthDateRange();

  // 优先使用 shortName + 日期
  const finalFileName = `${personInfo.shortName}${dateRange}`;
...
const generateGroupedExcelFiles = async () => {
  ...
  try {
    ...
    for (const companyGroup of groupInfo) {
      ...
      console.log(
        `生成文件: ${latestFileName}.xlsx，人员: ${companyGroup.groupName}`
      );
      ...
      // 获取人员配置信息
      const personInfo = personConfig.getPersonInfo(companyGroup.groupName);
      console.log(`人员配置信息:`, personInfo);
        ...
        row.eachCell((cell, colNumber) => {
          ...
          if (typeof cellValue === "string") {
            ...
            if (cellValue.includes("收件方：")) {
              cellValue = cellValue.replace(
                /收件方：[^，,\n]+/,
                `收件方：${personInfo.fullName}`
              );
              hasChanges = true;
            }

            // 替换联系人信息
            if (cellValue.includes("收件人：") && personInfo.contact) {
              cellValue = cellValue.replace(
                /收件人：[^，,\n]+/,
                `收件人：${personInfo.contact}`
              );
              hasChanges = true;
            }

            // 替换手机号信息
            if (cellValue.includes("电话：") && personInfo.phone) {
              // 获取当前电话
      ...
      if (companyGroup.hotelInfo && allSheetData.value.hotel) {
        ...
        const personKeyword = "预订人";
        const personColumnIndex = (hotelData[2] as any[]).findIndex(
          (cell: any) => cell && cell.toString().includes(personKeyword)
        );
        ...
        if (personColumnIndex !== -1) {
          ...
          // 筛选该人员的酒店数据
          const personHotelData = hotelData
            .slice(3)
            .filter((row: any[], index: number) => {
              const personValue = row[personColumnIndex];
              ...
              // 大梦龙途分组逻辑：根据人名匹配
              let groupName: string;
              const personName = personText.trim();
              if (personName === "陈敏铷") {
                groupName = "陈敏铷";
              } else if (personName === "洪晴") {
                groupName = "洪晴";
          ...
          const newData = [...headerRows, ...personHotelData];

          console.log(`  酒店工作表: ${newData.length} 行数据`);

          // 应用样式和格式
          await applyWorksheetStyling(newWorksheet, newData, personKeyword);
        }
      }
      ...
      if (
        ...
        if (personColumnIndex !== -1) {
          ...
          // 筛选该人员的国际酒店数据
          const personInternationalHotelData = internationalHotelData
            .slice(3)
            .filter((row: any[], index: number) => {
              const personValue = row[personColumnIndex];
              if (!personValue) return false;
          ...
          const newData = [...headerRows, ...personInternationalHotelData];
      ...
      if (companyGroup.trainInfo && allSheetData.value.train) {
        ...
        const personKeyword = "预订人";
        const personColumnIndex = (trainData[2] as any[]).findIndex(
          (cell: any) => cell && cell.toString().includes(personKeyword)
        );
        ...
        if (personColumnIndex !== -1) {
          ...
          // 筛选该人员的火车票数据
          const personTrainData = trainData
            .slice(3)
            .filter((row: any[], index: number) => {
              const personValue = row[personColumnIndex];
              if (!personValue) return false;
          ...
          const newData = [...headerRows, ...personTrainData];
      ...
      if (companyGroup.flightInfo && allSheetData.value.flight) {
        ...
        const personKeyword = "预订人";
        const personColumnIndex = (flightData[2] as any[]).findIndex(
          (cell: any) => cell && cell.toString().includes(personKeyword)
        );
        ...
        if (personColumnIndex !== -1) {
          ...
          // 筛选该人员的机票数据
          const personFlightData = flightData
            .slice(3)
            .filter((row: any[], index: number) => {
              const personValue = row[personColumnIndex];
              if (!personValue) return false;
          ...
          const newData = [...headerRows, ...personFlightData];
      ...
      if (
        ...
        if (personColumnIndex !== -1) {
          ...
          // 筛选该人员的国际机票数据
          const personInternationalFlightData = internationalFlightData
            .slice(3)
            .filter((row: any[], index: number) => {
              const personValue = row[personColumnIndex];
              if (!personValue) return false;
          ...
          const newData = [...headerRows, ...personInternationalFlightData];
      ...
      const getServiceFeeTotal = (worksheetName: string): number | null => {
        ...
        for (let col = 1; col < thirdRow.length; col++) {
          ...
          if (
            ...
            cellValue.includes("服务费")
          ) {
            serviceFeeColIndex = col;
            console.log(
              `在第${col + 1}列找到包含"服务费"的单元格: ${cellValue}`
            );
            break;
          }
        }
        ...
        let totalAmount = 0;
        console.log(
          `开始对第${serviceFeeColIndex + 1}列的数据求和（从第4行开始）`
        );
      ...
      const getWorksheetNameByExpenseType = (
        expenseType: string
      ): string | null => {
      ...
      const processExpenseTypeRow = (
        ...
        console.log(
          `第${rowIndex}行第6列原始值: ${originalValue} (类型: ${typeof originalValue})`
        );
            ...
            if (domesticFlightWorksheet) {
              ...
              if (totalAmount !== null) {
                ...
                if (targetColIndex !== -1) {
                  ...
                  console.log(
                    `替换第九行第${targetColIndex}列（总计金额）: ${oldValue} -> ${totalAmount}`
                  );
                } else {
      ...
      <div v-if="showData && excelData.length > 0" class="data-section">
        ...
        <div class="data-table">
          <el-table :data="getGroupInfo()" border style="width: 100%">
            <el-table-column prop="groupName" label="人员姓名" width="200" />
